...
export default function KanbanBoardContainer({ initialTasks }: { initialTasks: ProjectTaskProps[] }) {
    ...
    return (
        <DragDropContext onDragEnd={onDragEnd}>
            <div className="flex gap-2 py-4 overflow-x-auto w-full h-full  [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-muted-foreground/30">
                {columns.map((column) => (
